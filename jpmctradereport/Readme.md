...
## The Instruction
The **DataRow** is a model which describes an instruction to buy or sell given from various clients to the bank.
...
The **TradeSettlementDateCalculator** static class uses the proper strategy based on the given currency of an instruction.

## Statistics
The **TradeSettlementStatsCalculator** class is responsible of generating statistics based on a set of given instructions.
...
then sort by the USD trade amount and create a **TradeRank** object for each element.
...
## Demo
In order to show its usage there is a **TradeReportDataInitializer** class that generates a set of dummy instructions. 
